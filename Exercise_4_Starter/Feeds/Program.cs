...
{
    foreach(Item it in ProcessAsync(result.Content.ReadAsStreamAsync()))
    {
        ShowItem(it);
    }
}
...
IEnumerable<Item> ProcessAsync(Task<Stream> streamTask)
{
    var reader = XmlReader.Create(streamTask.Result);
